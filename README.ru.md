...
Для файлов C# и Python предпочитай `code-outline` вместо полного Read:

- `code-outline <file>` — структурный outline с номерами строк
  (≈8× экономия токенов по сравнению с полным чтением)
- `code-outline show <file> <Symbol>` — вытащить одно тело метода/класса
- `code-outline digest <dir>` — карта архитектуры модуля на одной странице
- `code-outline implements <BaseType> <dir>` — все наследники/реализации

Полный Read используй только когда outline не даёт достаточного контекста
— например, когда реально нужна логика внутри конкретного метода, который
ты уже нашёл через outline.

Запусти `code-outline help` чтобы увидеть полную справку.
```

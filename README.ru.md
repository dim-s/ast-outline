...
> сигнатуры с номерами строк, но **без тел методов**. Плюс структурный
> code-grep с аннотациями scope и kind. Сделан для LLM-агентов, которым
> нужно понять *форму* файла раньше, чем читать его целиком.
>
> Название вдохновлено [`ast-grep`](https://github.com/ast-grep/ast-grep) —
> оба инструмента поверх tree-sitter, но решают разные задачи: ast-grep
> переписывает код по структурным паттернам, ast-outline — карта и
> поиск по символам для чтения человеком или агентом.
...
1. **Экономия токенов — типично в 2–10 раз.** Outline заменяет полное
...
| ---        | --- |
| C#         | `.cs` |
| C++        | `.cpp`, `.cc`, `.cxx`, `.c++`, `.h`, `.hpp`, `.hh`, `.hxx`, `.h++`, `.ipp`, `.tpp`, `.inl`, `.cppm`, `.ixx` *(вкл. Unreal Engine)* |
| Python     | `.py`, `.pyi` |
| TypeScript | `.ts`, `.tsx` |
...
| SQL        | `.sql` *(таблицы с колонками, view, типы, enum'ы, функции, процедуры, триггеры, индексы, sequence, схемы, domain; PostgreSQL основной, MySQL/SQLite рабочие)* |
...
- **SQL** — фокус на DDL: `CREATE TABLE` (каждая колонка — отдельный `KIND_FIELD` child с source-true сигнатурой `id INTEGER PRIMARY KEY`, `email TEXT NOT NULL UNIQUE`); `CREATE VIEW` и `CREATE MATERIALIZED VIEW` различаются через `native_kind`; `CREATE TYPE foo AS (...)` (composite → `KIND_RECORD`); `CREATE TYPE foo AS ENUM (...)` (enum с member-children); `CREATE FUNCTION` с полным списком параметров и return-типом в сигнатуре; `CREATE TRIGGER` (`native_kind="trigger"`); `CREATE INDEX`, `CREATE SEQUENCE` (`KIND_FIELD` + `native_kind`); `CREATE SCHEMA` (`KIND_NAMESPACE`). `--` и `/* … */` комментарии перед statement прикрепляются к `docs`. `CREATE EXTENSION` идёт в imports. PL/pgSQL-тела внутри `AS $$ … $$` — opaque dollar-quoted strings; заголовок функции извлекается чисто, ошибки парсера внутри тел исключены из `error_count`. **Regex-fallback** восстанавливает 4 конструкции, которые upstream-грамматика не парсит: `CREATE [OR REPLACE] PROCEDURE` (`KIND_FUNCTION` + `native_kind="procedure"`), `CREATE DOMAIN` (`KIND_FIELD` + `native_kind="domain"`), `LOAD 'lib'` и `IMPORT FOREIGN SCHEMA …` (в imports). Fallback line-anchored и защищён AST-derived skip-зонами (`comment` / `marginalia` / `literal` / `block`) — приманки вида `CREATE PROCEDURE` в комментарии, в строковом литерале или в теле внешней функции не дают ложных деклараций. Покрытие диалектов: **PostgreSQL** — основной target (все современные конструкции работают); **MySQL** и **SQLite** — таблицы / колонки / индексы / views извлекаются чисто, есть `error_count > 0` шум от диалект-специфичных конструкций (`ENGINE=InnoDB`, `AUTOINCREMENT`, inline `KEY`); **MSSQL / T-SQL**, **Oracle PL/SQL**, **BigQuery** — частичное покрытие, bracketed-идентификаторы, `GO`-разделители, `VARCHAR2`, `STRUCT<>` / backtick-имена деградируют. На борту — `tree-sitter-sql` (DerekStride).
...
Для файлов `.cs`, `.cpp`, `.cc`, `.cxx`, `.h`, `.hpp`, `.hh`, `.py`,
`.pyi`, `.ts`, `.tsx`, `.js`, `.jsx`, `.java`, `.kt`, `.kts`, `.scala`,
`.sc`, `.go`, `.rs`, `.php`, `.phtml`, `.rb`, `.rake`, `.gemspec`,
`.css`, `.scss`, `.md` и `.yaml`/`.yml` сначала читай структуру через
`ast-outline`, а не полное содержимое.
...
2. **Структура файлов** — `ast-outline <paths…>`: сигнатуры с
   диапазонами строк, без тел (в 2–10 раз меньше токенов на нетривиальных
...
3. **Одно тело метода / типа / markdown-заголовка / yaml-ключа** —
   ...
   Для css/scss символ — это токен селектора (`.btn-primary`, `$var`,
   `%placeholder`); псевдоклассы и атрибутные фильтры отбрасываются,
   поэтому `.btn-primary` находит правило, даже если оно с `:hover`
   или вложено в `.modal`.
...
вывод cold, без подгруженного `ast-outline prompt`. Легенда
**динамическая** — перечисляются только те токены, которые реально
встретились в теле вывода. Для batched-дайджестов из одних YAML или
markdown файлов (где нет callable, нет kind-тегов, нет наследования)
легенда не выводится вообще; для кодовых batched-дайджестов остаётся
только то подмножество, которое реально используется. Токены следуют
...
Java, Kotlin, Scala, Go, Rust, PHP, Ruby, CSS, SCSS, SQL, Markdown, YAML), языко-агностичные рендереры, поиск по

...
```markdown
## Изучение кода — для C# / Python используй `code-outline`

Прежде чем открывать файл `.cs`, `.py` или `.pyi`, запусти `code-outline`
чтобы увидеть его структуру. Полное чтение файла — только когда ты уже
знаешь, какое именно тело метода тебе нужно.

Workflow (останавливайся на том шаге, который ответил на твой вопрос):

1. **Незнакомая директория или модуль** — `code-outline digest <dir>`
   выдаёт классы и публичные методы каждого файла на одной странице.

2. **Один файл, структура** — `code-outline <file>` печатает сигнатуры
...
4. **Кто наследует / реализует тип** — `code-outline implements
   <TypeName> <dir>`, на AST и без ложных срабатываний; `grep` для этого
   не нужен.

Откатывайся на полное чтение файла только когда `show` дал сигнатуру, но
нужен окружающий контекст. Диапазон `L<start>-<end>` в outline — это
готовый offset для твоего read-инструмента, если он такое поддерживает.

`code-outline help` — флаги и менее частые опции.
```
...
с номерами строк `L<start>-<end>`, doc-комментарии сохраняются.
